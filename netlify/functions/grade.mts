...
export default async (req: Request, _context: Context) => {
  ...
  try {
    if (!process.env.GEMINI_API_KEY) {
      return json(
        { error: "Gemini API kaliti sozlanmagan. Iltimos, GEMINI_API_KEY environment variable qo'shing." },
        { status: 500 },
      );
    }

    const { imageBase64, mimeType } = (await req.json()) as GradeRequest;
    ...
    if (sizeInBytes > MAX_IMAGE_BYTES) {
      return json(
        { error: "Fayl hajmi juda katta. Iltimos, 15MB dan kichik rasm yuklang." },
        { status: 413 },
      );
    }
    ...
    const statusCode = message.includes("API kaliti noto'g'ri") ? 401 : 500;

    console.error("Error evaluating homework:", message);
