...
import { X, Moon, Sun, User, Settings, PieChart } from 'lucide-react';
...
export function ProfileModal({ isOpen, onClose, history, isDarkMode, toggleDarkMode, username, onLogout }: ProfileModalProps) {
      ...
      <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto flex-1 flex flex-col gap-6">
          ...
          <div className="flex items-center gap-4">
            <div className="h-16 w-16 rounded-full bg-indigo-100 dark:bg-indigo-900/50 flex items-center justify-center border-2 border-indigo-200 dark:border-indigo-800">
              <User className="h-8 w-8 text-indigo-600 dark:text-indigo-400" />
            </div>
            <div>
              <h3 className="text-xl font-bold text-slate-900 dark:text-white">{username}</h3>
              <p className="text-sm text-slate-500 dark:text-slate-400">Math AI Explorer</p>
            </div>
          </div>
