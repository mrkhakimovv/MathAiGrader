...
export async function evaluateHomework(images: { imageBase64: string, mimeType: string }[]) {
...
1. Carefully transcribe the student's entire solution, line by line. Use LaTeX enclosed in $ for inline math and $$ for block math (e.g. $x^2 + y^2 = z^2$).
   - CRITICAL: Write each separate equation, mathematical step, or distinct line of text from the image on a NEW LINE in your transcription. Use double newlines between each line to ensure they render as separate blocks in Markdown. Do not run multiple steps or equations together on the same line.
...
7. IMPORTANT FOR MULTIPLE IMAGES: If multiple images are provided, you MUST structure your \`transcription\`, \`feedback\`, and \`errorSteps\` to address each image separately. Start the section for each image with its number, like "1-rasm.\\n\\n[content for image 1]\\n\\n2-rasm.\\n\\n[content for image 2]" and so on.
